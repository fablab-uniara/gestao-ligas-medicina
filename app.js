...
let currentUser = null; // Guarda os dados de quem está logado
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        try {
            ...
            // REGRA: Coordenador (Admin Global)
            ...
            // REGRA: Aluno Comum (Acesso Local)
            if (userDoc.exists()) {
...
// --- 3. CARREGAMENTO, FILTRAGEM E SALVAMENTO ---
...
// Filtra os dados: Admin vê TUDO. Usuário comum vê apenas o que ele próprio criou.
...
async function renderPage(page) {
    ...
    // Títulos das colunas dinâmicos (Admin vê quem fez)
        ...
        case 'dashboard':
            ...
            content.innerHTML = `
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                    <div class="card" style="border-top: 4px solid var(--color-primary);"><h3>Ligas</h3><p style="font-size:28px; color:var(--color-primary); font-weight:bold;">${getDadosPermitidos('ligas').length}</p></div>
                    <div class="card" style="border-top: 4px solid #00695c;"><h3>Pesquisas</h3><p style="font-size:28px; color:#00695c; font-weight:bold;">${getDadosPermitidos('pesquisas').length}</p></div>
                    <div class="card" style="border-top: 4px solid #e65100;"><h3>Eventos</h3><p style="font-size:28px; color:#e65100; font-weight:bold;">${getDadosPermitidos('eventos').length}</p></div>
                    <div class="card" style="border-top: 4px solid #4a148c;"><h3>Projetos</h3><p style="font-size:28px; color:#4a148c; font-weight:bold;">${getDadosPermitidos('extensao').length}</p></div>
                </div>`;
            break;
...
// Renderiza as tabelas com botões de Ação (Editar, Relatório, Excluir)
    ...
    if (data.length === 0) {
        ...
        data.forEach(item => {
            ...
            headers.forEach(h => {
                ...
                if(key === 'autor') key = 'autorNome'; // Mapeia a coluna Autor
                html += `<td>${item[key] || '---'}</td>`;
            });
            
            // Qual função de edição chamar?
...
// Recupera dados do formulário ou cria um objeto novo
function obterObjetoExistente(type, id) {
    if (!id) return { id: Date.now(), autorEmail: currentUser.email, autorNome: currentUser.nome };
    return db[type].find(i => i.id === id); // Se id existir, puxa do banco
}

// Funções de Modal adaptadas para Edição (O parâmetro id é opcional)
function openLigaModal(id = null) { 
    const item = id ? db.ligas.find(i => i.id === id) : {};
...
function openPesquisaModal(id = null) { 
    const item = id ? db.pesquisas.find(i => i.id === id) : {};
...
function openEventoModal(id = null) { 
    const item = id ? db.eventos.find(i => i.id === id) : {};
...
function openExtensaoModal(id = null) { 
    const item = id ? db.extensao.find(i => i.id === id) : {};
...
function openAtividadeModal(id = null) { 
    const item = id ? db.atividades.find(i => i.id === id) : {};
...
// --- 7. FUNÇÕES DE SALVAMENTO (CREATE & UPDATE) ---
async function finish(page) { await saveDb(); renderPage(page); closeActiveModal(); }

function pushOrUpdate(type, id, novoDado) {
    if (id) {
        // Se tem ID, estamos editando. Acha o índice e substitui mantendo autor original.
        const idx = db[type].findIndex(i => i.id === id);
        if (idx !== -1) {
            novoDado.id = db[type][idx].id;
            novoDado.autorEmail = db[type][idx].autorEmail; // Preserva autor original
...
// Relatórios mantidos inalterados na lógica principal
